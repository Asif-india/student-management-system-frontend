...
const subjectSchema = z.object({
  name: z.string().min(2, 'Subject name must be at least 2 characters').max(100, 'Subject name cannot exceed 100 characters').optional(),
  code: z.string().min(1, 'Subject code is required').max(20, 'Subject code cannot exceed 20 characters').optional(),
  ...
  useEffect(() => {
    if (subjectData?.data) {
      reset({
        ...
        credits: subjectData.data.credits,
        type: subjectData.data.type,
        grade: subjectData.data.grade,
        status: subjectData.data.status,
      })
    }
  }, [subjectData, reset])

  const mutation = useMutation({
    mutationFn: (data: { id: string; subjectData: any }) =>
      subjectService.updateSubject(data.id, data.subjectData),
    onSuccess: (response) => {
    ...
    onError: (error: any) => {
      notifyError(error.response?.data?.message || 'Failed to update subject')
    },
  })

  const onSubmit = (data: SubjectFormData) => {
    mutation.mutate({ id: id!, subjectData: data })
  }

  if (isLoading) {
    return <LoadingState message="Loading subject data..." />
  }

  if (error) {
    return <ErrorState message="Failed to load subject data" onRetry={() => window.location.reload()} />
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Edit Subject</h1>
          <p className="text-text-secondary mt-1">Update subject information</p>
        </div>
      </div>
